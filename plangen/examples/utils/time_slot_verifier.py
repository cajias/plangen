...
class TimeSlotVerifier:
    ...
    def is_valid_meeting_slot(
        ...
        if time_slot.start < self.day_start:
            return False, f"Meeting starts before working hours ({self.day_start//60}:00)"
        ...
        if duration and time_slot.duration() != duration:
            return False, f"Meeting duration ({time_slot.duration()} min) doesn't match required duration ({duration} min)"
