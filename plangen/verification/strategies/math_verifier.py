...
class MathVerifier(BaseVerifier):
    ...
    def __init__(self):
        ...
        self.domain_keywords = [
            "calculate", "compute", "solve", "equation", "math", "arithmetic",
    ...
    def verify_solution(
        ...
        if answer is not None:
            if expected_answer is not None:
                ...
                score = 80 if steps_valid else 40  # Lower confidence without expected answer
    ...
    def _compare_answers(self, answer: float, expected: float, tolerance: float = 0.001) -> bool:
    ...
    def _extract_numerical_constraints(self, problem_statement: str) -> List[str]:
        ...
        precision_pattern = r'(?:round|precision|decimal places|significant figures).*?(\d+)'
        match = re.search(precision_pattern, problem_statement.lower())
        if match:
            precision = match.group(1)
            constraints.append(f"The answer must be rounded to {precision} decimal places.")
