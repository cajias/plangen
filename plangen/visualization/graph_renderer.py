...
class GraphRenderer(PlanObserver):
    ...
    def _update_generic_graph(self, plan_data: Dict[str, Any]) -> None:
        ...
        prev_node_id = f"update_{self.update_count-1}" if self.update_count > 0 else None
        
        # Add node with timestamp and all data
        self.graph.add_node(
            node_id,
            timestamp=time.time(),
            **{k: v for k, v in plan_data.items() if isinstance(v, (str, int, float, bool))}
        )
    ...
    def render(
        ...
        for node in self.graph.nodes:
            ...
            if self.algorithm_type == "TreeOfThought":
                ...
                steps_str = str(node_data.get("steps", []))[:30] + "..." if "steps" in node_data else ""
                ...
                feedback = node_data.get("feedback", "")[:20] + "..." if "feedback" in node_data else ""
        ...
        plt.title(f"{self.algorithm_type or 'Unknown'} Plan Exploration - {time.strftime('%Y-%m-%d %H:%M:%S')}")
        
        # Save the figure if requested
        if save:
            if not filename:
                timestamp = time.strftime("%Y%m%d%H%M%S")
                filename = f"{self.algorithm_type or 'plan'}_{timestamp}.{self.render_format}"
            
            filepath = os.path.join(self.output_dir, filename)
            plt.savefig(filepath, format=self.render_format, dpi=300, bbox_inches="tight")
