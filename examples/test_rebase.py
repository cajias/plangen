...
def main():
    ...
    try:
        ...
        pruned_nodes = metadata["nodes_explored"] - len([
            node for node in metadata["all_nodes"] 
        ...
        for depth in range(max_depth_found + 1):
            depth_nodes = [node for node in metadata["all_nodes"] if node["depth"] == depth]
            ...
            for i, node in enumerate(depth_nodes):
                ...
                pruned = "🗑" if node["score"]/100 < metadata["pruning_threshold"] else " "
