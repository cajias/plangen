...
class TestConstraintAgent(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.agent = ConstraintAgent(model=self.mock_model, prompt_manager=self.mock_prompt_manager)
    
    def test_run_extracts_constraints(self):
        """Test that run method extracts constraints correctly."""
        # Mock prompt manager responses
        self.mock_prompt_manager.get_system_message.return_value = "You are a constraint extraction agent"
        self.mock_prompt_manager.get_prompt.return_value = "Extract constraints from: {problem}"
        ...
        self.mock_prompt_manager.get_system_message.assert_called_once_with("constraint")
